<script>
    ...
    const pageRight = () => {
        currentPage =
            currentPage == pageNums.length ? currentPage : currentPage + 1;
    };
    ...
    const captureUniqueItemsToFilter = (items) => {
        ...
        for (let item of items) {
            ...
            for (const key of filterableKeys) {
                iterations++;
                if (filterMap[key]) {
                    if (filterMap[key].indexOf(item[key]) >= 0) {
                        //duplicated value for the key
                        continue;
                    }
    ...
    function stickyHeader(lastKnownScrollPosition) {
        count++
        console.log('called'+scrollTopLast);
        let thElements = Array.from(table.children[0].children[0].children)
        // console.log(thElements);
        for(const el of thElements){
            el.style.transform = "translateY("+ scrollTopLast +"px)"
        }
        // console.log(Array.from(thElements));
    }
    ...
    onMount(() => {
        ...
        scrollableTableContainer.addEventListener("scrollx", () => {
            scrollTopLast = scrollableTableContainer.scrollTop;
            if (!ticking) {
        ...
        scrollableTableContainer.addEventListener("scrollx", () => {
            ...
            if (!scrollWait) {
                ...
                lastCall = setTimeout(() => {
                    if (!autoScrolled) {
                        const tabelRowHeight =
                            table.children[1].children[0].offsetHeight; //table -> body -> tr
                        ...
                        if (
                            scrollDirection == "up" &&
                            scrollableTableContainer.scrollTop < 1
                        ) {
                            ...
                            if (currentPage != 1) {
                                ...
                                setTimeout(() => {
                                    //Jump scrollbar
                                    let maxScrollableTop =
                                        scrollableTableContainer.scrollHeight -
                                        scrollableTableContainer.clientTop;
                                    scrollableTableContainer.scrollTop =
                                        maxScrollableTop - scrollEdgeOffset;
                                }, 80);
                            }
                        } else if (
                            scrollDirection == "down" &&
                            Math.round(
                                scrollableTableContainer.scrollTop +
                                    scrollableTableContainer.offsetHeight
                            ) >= scrollableTableContainer.scrollHeight
                        ) {
                            //scroll reached end
    ...
    const openFilter = (clickedElement) => {
        ...
        if (clickedElement.children.length > 1) {
            ...
            let templateMenu =
                outerMostContainerDiv.getElementsByClassName("filter-menu")[0];
...
<div
    class="columns is-multiline data-list-container"
    bind:this={outerMostContainerDiv}
>
    <input
        type="text"
